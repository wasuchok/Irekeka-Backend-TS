...
export const getDashboardStats = async (req: Request, res: Response) => {
  ...
  try {
    const [stats]: any = await sequelize.query(
      `
        SELECT
          (SELECT COUNT(*) FROM [ITDB].[dbo].[tb_Irekeka_Stock] WHERE en = 1) AS totalEquipments,
          (SELECT COUNT(*) FROM [ITDB].[dbo].[tb_Irekeka_Stock] WHERE status = 'borrowed' AND en = 1) AS borrowed,
          (
            SELECT COUNT(*)
            FROM [ITDB].[dbo].[tb_Irekeka_Record]
            WHERE date_in IS NULL
              AND DATEADD(DAY, ISNULL(num_date, :defaultDue), date_out) < GETDATE()
          ) AS lateReturns,
          (
            SELECT COUNT(*)
            FROM [ITDB].[dbo].[tb_Irekeka_Stock]
            WHERE en = 1
  ...
  try {
    const rows = (await sequelize.query(
      `
        ...
        FROM [ITDB].[dbo].[tb_Irekeka_Record] r
        INNER JOIN [ITDB].[dbo].[tb_Irekeka_Stock] s ON r.code = s.equipment_code
    ...
    const rows = (await sequelize.query(
      `
        ...
        CROSS APPLY (
          ...
          WHERE acts.action_date IS NOT NULL
          ORDER BY acts.action_date DESC
        ) AS recent
        WHERE recent.action_date IS NOT NULL
        ORDER BY recent.action_date DESC, r.seq DESC
      `,
      { type: QueryTypes.SELECT }
    )) as Array<{
      seq: number;
